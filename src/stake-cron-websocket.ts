...
const validateEnv = (): { privateKey: Address; stakingContract: Address } => {
    ...
    const stakingContractRaw = process.env.STAKING_CONTRACT_ADDRESS
...
const walletClient = createWalletClient({
    account,
    chain: mainnet,
    transport: webSocket(`wss://mainnet.infura.io/ws/v3/${process.env.INFURA_PROJECT_ID}`),
...
const checkAndStake = async () => {
    try {
        ...
        if (availableSpace <= 0 || linkBalanceInLink <= 0) return
        ...
        walletClient
            ...
            .then((receipt) => logger.info('Tx Confirmed:', receipt.status))
...
const startCron = async () => {
    ...
    const loop = async () => {
        ...
        const delay = Math.max(0, CHECK_INTERVAL_MS - elapsed)
...
startCron().catch((err) => logger.error('Cron Error:', err))
